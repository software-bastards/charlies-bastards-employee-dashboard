...
const { isEmpty } = require("../Configurations/helper");
...
router.post(
  ...
  (req, res) => {
    ...
    if (isNotNumber(req.body.month_number)) {
      ...
      if (isNotNumber(req.body.hour)) {
        ...
        if (isNotNumber(req.body.day_number)) {
          ...
          if (isEmpty(req.body.day_number)) {
            res
              .status(400)
              .send({ message: "You can not leave this field empty!" });
          } else {
            if (isEmpty(req.body.month_number)) {
              res
                .status(400)
                .send({ message: "You can not leave this field empty!" });
            } else {
              if (isEmpty(req.body.hour)) {
                res
                  .status(400)
                  .send({ message: "You can not leave this field empty!" });
              } else {
